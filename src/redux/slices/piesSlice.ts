...
export const fetchPies = createAsyncThunk<ItemType[],PropsType>('pies/fetchPies', async (props) => {
  const { data } = await axios.get<ItemType[]>(
    ...
    }${props.searchValue ? `&search=${props.searchValue}` : ``}&limit=11&page=${props.currentPage}`,
  );
  return data;
});

export const fetchItemById = createAsyncThunk('pies/fetchPie', async (id:number) => {
  const { data } = await axios.get<ItemType[]>(`https://63defbd13d94d02c0bb2ee5d.mockapi.io/pies?id=${id}`);
